...
function ProgramList({ onSelectProgram }) {
  ...
  const handleAddProgram = async (e) => {
    ...
    try {
      const programData = { title: newProgramTitle };
      await createProgram(programData); // Appeler l'API pour créer
  ...
  if (error) {
    ...
    return (
      <div>
        ...
        <ProgramForm onSubmit={handleAddProgram} title={newProgramTitle} setTitle={setNewProgramTitle} />
      ...
      {programs.length === 0 ? (
        ...
        <ul>
          {programs.map((program) => (
            <li key={program.id} style={{ marginBottom: '5px' }}>
              {editingId === program.id ? (
                <>
                  ...
                  <button onClick={() => handleEditSave(program.id)} disabled={saving} style={{color: 'green', marginRight: 4}}><FaCheck /></button>
                  <button onClick={handleEditCancel} disabled={saving} style={{color: 'red'}}><FaTimes /></button>
                </>
              ) : (
                <>
                  <span
                  ...
                  <button onClick={() => handleEditClick(program)} style={{marginRight: 8}}><FaPencilAlt /></button>
                  <button
